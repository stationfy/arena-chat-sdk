...
export class LiveConfig {
    private static configInstance: LiveConfig;
    ...
    constructor(env: ConfigProperties) {
        ...
        LiveConfig._enviroment = env === 'EU' ? this.enviroments.EU :  this.enviroments.USA; 
    }
  
    public static get instance(): LiveConfig {
      if (!LiveConfig.configInstance) {
        LiveConfig.configInstance = new LiveConfig('USA');
      }else{
        console.log('Config instace not created...')
      }
  
      return LiveConfig.configInstance;
    }

    public static get enviroment(): EnvType | null {
        return LiveConfig._enviroment;
    }
