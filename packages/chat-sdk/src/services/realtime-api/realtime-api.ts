...
export class RealtimeAPI {
  ...
  private static realtimeApiStrategy: BaseRealtimeAPI;
  ...
  private static async defineStrategy(
    ...
    dataPath: string,
    site?: Site,
  ): Promise<BaseRealtimeAPI> {
    RealtimeAPI.close();
    if (site?.featureFlags?.['chat_fireproxy']?.status === 'active') {
      const { RealtimeApiWS } = await import('./realtime-api-ws');
      return new RealtimeApiWS(chatroomId, channelId, () => RealtimeAPI.fallback(dataPath));
    } else {
      const { RealtimeApiFirestore } = await import('./realtime-api-firestore');
      return new RealtimeApiFirestore(dataPath);
    }
  }

  private static async fallback(dataPath: string): Promise<void> {
    Logger.instance.log('error', 'LiveblogServices: Starting Arena WS Fallback');
    const { RealtimeApiFirestore } = await import('./realtime-api-firestore');
    RealtimeAPI.realtimeApiStrategy = new RealtimeApiFirestore(dataPath);
  }
  ...
  public static async getInstance(
    ...
    if (!RealtimeAPI.instances[chatroomId]) {
      const strategy = await RealtimeAPI.defineStrategy(chatroomId, channelId, dataPath, site);
      RealtimeAPI.realtimeApiStrategy = strategy;
      RealtimeAPI.instances[chatroomId] = new RealtimeAPI();
    }
  ...
  public static close(): void {
    if (RealtimeAPI.realtimeApiStrategy) {
      return RealtimeAPI.realtimeApiStrategy.close();
    }
  }
  ...
  public listenToMessage(callback: (message: ChatMessage) => void): () => void {
    return RealtimeAPI.realtimeApiStrategy.listenToMessage(callback);
  }
  ...
  public async fetchRecentMessages(limit: number): Promise<ChatMessage[]> {
    return RealtimeAPI.realtimeApiStrategy.fetchRecentMessages(limit);
  }
  ...
  public async fetchPreviousMessages(limit: number, before: number): Promise<ChatMessage[]> {
    return RealtimeAPI.realtimeApiStrategy.fetchPreviousMessages(limit, before);
  }
