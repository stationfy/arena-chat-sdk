...
const DEFAULT_LOAD_MESSAGES_COUNT = 10;
export class Channel implements BaseChannel {
  ...
  private totalLimit = DEFAULT_LOAD_MESSAGES_COUNT;
  ...
  private async fetchRecentMessages(limit = 20): Promise<ChatMessage[]> {
    const realtimeInstance = await this.getRealtimeAPIInstance();
    return realtimeInstance.fetchRecentMessages(limit);
  }
   ...
   */
  public async loadPreviousMessages(limit = DEFAULT_LOAD_MESSAGES_COUNT): Promise<ChatMessage[]> {
    ...
    try {
      ...
      const previousMessages = await (
        await this.getRealtimeAPIInstance()
      ).fetchPreviousMessages(limit, firstMessage.createdAt);
  ...
  /**
   *  Register listener of messages callback
   */
  private async listenToMessageCallback() {
    const realtimeInstance = await this.getRealtimeAPIInstance();
    this.messageModificationListenerUnsubscribe = realtimeInstance.listenToMessage((message) => {
  ...
  /**
   *  Remove first message from cached messages list
   */
  ...
  public watchChatConfigChanges(
    ...
    try {
      ...
      if (!this.chatConfigListenerUnsubscribe) {
        ...
        this.getRealtimeAPIInstance().then((realtimeInstance) => {
          this.chatConfigListenerUnsubscribe = realtimeInstance.listenToChatConfigChanges(path, (nextChatRoom) => {
  ...
  private async getRealtimeAPIInstance(): Promise<RealtimeAPI> {
    const response = getClientCache(`/chatroom/${Credentials.apiKey}/${this.chatRoom.slug}`) as {
      chatInfo: ChatRoom;
      publisher: Site;
