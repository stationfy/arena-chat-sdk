...
import { ArenaSocketMessage } from '@arena-im/chat-types';
...
export class ArenaSocket {
  ...
  private connect(): WebSocket {
    ...
    this.connectTimeout = window.setTimeout(() => this.handleTimeout(), CONNECTION_TIMEOUT);
  ...
  private pinPong(): void {
    ...
    this.pingPongTimeout = window.setTimeout(this.pinPong.bind(this), PING_PONG_TIMEOUT);
  }
  ...
  private addListeners(socket: WebSocket): void {
    ...
    socket.onmessage = (event) => {
      if (event.data === 'pong') {
        ...
        if (data.errMsg) {
          Logger.instance.log('error', `ArenaSocket message error: ${data.errMsg}`);
          return;
        }
        if (currentCallback) {
          const messages = data.payload.map((message: ArenaSocketMessage) => message.content);
